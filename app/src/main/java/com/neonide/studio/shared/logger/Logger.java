...
public class Logger {
    ...
    public static void logMessage(int logPriority, String tag, String message) {
        ...
        // Optional file logging for IDE debugging when logcat is not available.
        // Note: This is best-effort and silently ignored if it fails.
        if (logged) {
            try {
                Context ctx = com.neonide.studio.app.TermuxApplication.getApplicationContextUnsafe();
                if (ctx != null) {
                    IDEFileLogger.log(ctx, getFullTag(tag) + ": " + message);
                }

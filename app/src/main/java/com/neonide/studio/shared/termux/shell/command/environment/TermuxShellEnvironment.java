package com.neonide.studio.shared.termux.shell.command.environment;
...
import com.neonide.studio.shared.shell.command.environment.AndroidShellEnvironment;
import com.neonide.studio.shared.shell.command.environment.ShellEnvironmentUtils;
import com.neonide.studio.shared.shell.command.environment.ShellCommandShellEnvironment;
import com.neonide.studio.shared.termux.TermuxBootstrap;
import com.neonide.studio.shared.termux.TermuxConstants;
import com.neonide.studio.shared.termux.shell.TermuxShellUtils;
    ...
    public HashMap<String, String> getEnvironment(@NonNull Context currentPackageContext, boolean isFailSafe) {
        ...
        if (!isFailSafe) {
            ...
            if (TermuxBootstrap.isAppPackageVariantAPTAndroid5()) {
                ...
                // Termux binaries on Android 7+ typically rely on DT_RUNPATH, so LD_LIBRARY_PATH is unset by default.
                // However, for rebranded/forked package names (e.g. com.neonide.studio) or custom bootstraps,
                // DT_RUNPATH may not point to the correct $PREFIX/lib. Setting LD_LIBRARY_PATH ensures
                // core executables (like bash) can link against bootstrap libs such as libandroid-support.so.

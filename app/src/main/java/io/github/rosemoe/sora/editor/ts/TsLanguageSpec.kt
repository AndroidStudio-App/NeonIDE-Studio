...
open class TsLanguageSpec(
    ...
    /**
     * Optional query used to capture code blocks.
     *
     * Some android-tree-sitter builds throw if the query source is blank, so use [TSQuery.EMPTY]
     * when no blocks query is provided.
     */
    val blocksQuery: TSQuery = if (codeBlocksScmSource.isBlank()) TSQuery.EMPTY else TSQuery.create(language, codeBlocksScmSource)

    /**
     * Optional query used to capture brackets.
     *
     * Some android-tree-sitter builds throw if the query source is blank, so use [TSQuery.EMPTY]
     * when no brackets query is provided.
     */
    val bracketsQuery: TSQuery = if (bracketsScmSource.isBlank()) TSQuery.EMPTY else TSQuery.create(language, bracketsScmSource)

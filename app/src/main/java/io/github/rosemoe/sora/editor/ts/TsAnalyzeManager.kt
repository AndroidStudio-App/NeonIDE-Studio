...
open class TsAnalyzeManager(val languageSpec: TsLanguageSpec, var theme: TsTheme) :
    ...
    protected fun destroyPreviousRes() {
        // IMPORTANT:
        // We must stop the worker thread before closing TSQuery objects (done in TsLanguageSpec.close()).
        // Otherwise we can hit native SIGSEGV when TSQuery is closed while TSQueryCursor is iterating.
        val t = thread
        thread = null
        t?.let {
            if (it.isAlive) {
                it.abort = true
                it.interrupt()
                // Best-effort join to reduce race window. Keep timeout small to avoid UI hangs.
                runCatching { it.join(250) }
            }
        }
    ...
    inner class TsLooperThread : Thread() {
        ...
        fun updateStyles() {
            // Guard against races: languageSpec (and its TSQuery objects) may be closed when switching languages.
            // If we try to access a closed/invalid TSQuery here, tree-sitter throws "Cannot access native object".
            if (languageSpec.closed || !languageSpec.tsQuery.canAccess()) {
                return
            }
            ...
            val scopedVariables = try {
                ...
                // Native objects may be closing concurrently
                return
            } catch (_: IllegalArgumentException) {
                // Invalid query (shouldn't happen for valid spec, but can happen during disposal)
                return
            }
        ...
        fun updateCodeBlocks() {
            // NOTE:
            // TsLanguageSpec closes TSQuery objects when the language is destroyed/switched.
            // Accessing TSQuery#patternCount calls native checkAccess() and can throw
            // IllegalStateException("Cannot access native object") if the query is already closed.
            // So we must check canAccess() first and guard native calls.
            if (languageSpec.closed) {
                return
            }

            val blocksQuery = languageSpec.blocksQuery

            if (!blocksQuery.canAccess()) {
                return
            }

            val patternCount = try {
                blocksQuery.patternCount
            } catch (_: IllegalStateException) {
                return
            }
            ...
            TSQueryCursor.create().use { cursor ->
                cursor.exec(blocksQuery, tree!!.rootNode)
                    ...
                    if (languageSpec.blocksPredicator.doPredicate(
                        ...
                        match.captures.forEach {
                            val block = CodeBlock().also { block ->
                                ...
                                val captureName = try {
                                    blocksQuery.getCaptureNameForId(it.index)
                                } catch (_: IllegalStateException) {
                                    ""
                                }

                                val end = if (captureName.endsWith(".marked")) {
